...
% Path
save_path = 'C:\Users\mastaffs\Desktop\testset';
label_file = fopen(fullfile(save_path,sprintf('label_%d.txt',nSource)),'w');
var_file = fopen(fullfile(save_path, sprintf('var_%d.csv',nSource)),'w');
eval_file = fopen(fullfile(save_path, sprintf('eval_%d.csv',nSource)),'w');
for nt = 1:N_test
    fprintf('Test %d\n',nt)
%     rng(50*nt)
    rng(150*nt)
